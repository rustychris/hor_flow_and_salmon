...
parameters {
  ...
  // Parameters for heading -- trying this where the length of the vector gives the concentration
  // parameter and its angle the mean angle. 
  matrix[K,2] hdg_vec;
}
...
transformed parameters {
  ...
  vector[K] hdg_mean;
  for (k in 1:K) {
    hdg_conc[k]=sqrt(pow(hdg_vec[k,1],2) + pow(hdg_vec[k,2],2));
    if( hdg_conc[k] < 1e-5 ) {
      hdg_mean[k]=0;
    } else {
      hdg_mean[k]=atan2(hdg_vec[k,2],
                        hdg_vec[k,1]);
    }
  }
}
model {
  ...
  for (k in 1:K) {
    ...
    tmp = normal_lpdf( hdg_vec[k,1] | 10, 20)
      + normal_lpdf( hdg_vec[k,2] | -5 + 10*(k-1.0)/(K-1.0), 20);
    target += tmp;
