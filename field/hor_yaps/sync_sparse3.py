...
solution, but limits soundspeed to piecewise constant.

That allows for using the same apply sync methods in yaps.

In a quick test, this doesn't change the results.
"""
...
# With the sparse solver it can handle longer periods, at least with a small
# number of offsets.  But there isn't much gain, and it gets pretty slow.

t_sel_min=all_detections['epo'].min() + 0*3600
t_sel_max=t_sel_min+12*3600
detections=all_detections[ (all_detections['epo']>=t_sel_min)&(all_detections['epo']<t_sel_max) ].copy()
# detections=all_detections.copy() # whole hog
...
ncols=((n_off+1) * nh) + (xdata['n_ss_idx'])
...
for p in range(xdata['np']):
    ...
    for h2 in hi[1:]:
        ...
        # sparse3 change:
        row[ nh*(n_off+1) + ss_idx] = (xdata['dist_mat'][h1,tag_idx] - xdata['dist_mat'][h2,tag_idx])
...
while len(bad_pings)<200:
    ...
    if 1: # dense matrix approach
        soln,res,rank,sing=np.linalg.lstsq(Mslim[valid,:],
                                           b[valid],
                                           rcond=-1)
    else:
        # So far this seems not as good.
        # it labels twice as many pings as bad, and yet
        # the resulting errors are still greater.
        # It was stopping on the max number of iterations, 128.
        # bumping up the number of iterations and it gets the same
        # results as lstsq().
        Msparse=sparse.csr_matrix(Mslim[valid,:])
        soln,istop,itn,r1,r2,anorm,acond,arnorm,xnorm,v = sparse.linalg.lsqr(Msparse,b[valid],
                                                                             iter_lim=4*Msparse.shape[1])
        # lsmr was much worse, though didn't do any tweaking..
...
# what does yaps use from the sync model?
# or is it better to write my own apply sync?

# Very easy to just skip the sync step in yaps ---
# just copy epo+frac to eposync.
# 

# Before doing that, though, can I use a sparse matrix lsq solver?
# and if so, can I then process a longer time period?
