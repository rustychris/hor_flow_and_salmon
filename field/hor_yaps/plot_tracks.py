...
del df['track']
df.rename(columns={'trimmed':'track'},inplace=True)
...
all_tracks=[ track.iloc[:-1,:]
             for track in df['track'].values ]

# sort shouldn't be necessary, but hiccups in the screening can
# cause the columns to appear in different orders, so best to
# sort
seg_tracks=pd.concat( all_tracks, sort=True)
...
def fig_track_swimming(seg_track,num=20,zoom=None,buttons=True,
    ...
    if smooth_win is not None:
        print("Janky Lowpass!  Should really fix the trilateration")
        xvals=filters.lowpass_fir(xvals,smooth_win)
        yvals=filters.lowpass_fir(yvals,smooth_win)
...
plots=fig_track_swimming(df_by_id.loc['7A96','track'],
                         num=20,smooth_win=5)
## 
