...
def plot_recon_time_series(ax,breaks,values,**kw):
    # mids=0.5*(ss_breaks[:-1] + ss_breaks[1:])
    val_expand=np.repeat(values,2)
    break_expand=np.repeat(breaks,2)[1:-1]
    return ax.plot( break_expand, val_expand, marker='o', ms=3, **kw)
