...
def my_log_prob_lin(vec,out=False):
    ...
    if 0: # Linear interpolation
        ...
        spl=UnivariateSpline( ss_breaks,kw['ss'],k=2 )
    ...
    if 0: # Linear
        ...
        for h in range(xdata['nh']):
            spl=UnivariateSpline( offset_breaks, kw['offset'][h,:],k=2 )
...
# even with the extra function evaluations it does no better.    
best=fmin_powell(my_log_prob_lin,best, # pack_parameters(init),
                 callback=update,maxfun=100000)
...
axs[2].hist(np.sqrt(calcs['errors']),bins=np.linspace(0.0,0.005,100))
...
# Oddly the splines(k=3) don't improve the results really at all.
# Presumably the dominant errors are in the locations of the
# hydrophones?

# Out of curiosity, what about k=2?
