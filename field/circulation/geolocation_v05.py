...
# now that I added drift (I think that's the cause), L-BFGS is stable
# there is something about L-BFGS having its history less than the number
# of free variables. Even now, there are only 3 free variables, and
# supposedly it defaults to a history of 5.  but maybe it just doesn't
# work well with the 2 variables I had been using.
# regardless, the model is working now.  should be easy enough to run it
# with 3 stations.
# then I can start to see circulation.
# may have to go to a cubic drift, or start to think about splines.   but
# first, just use linear, and can use shorter periods as needed (maybe by
# finding the period that gives the best standard error).
...
# 23us rmse
print(f"Mean error: {np.mean(errors):3f} ms  rmse: {np.sqrt(np.mean(np.array(errors)**2)):.3f} ms")
